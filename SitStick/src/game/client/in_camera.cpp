...
//-------------------------------------------------- Global Variables
ConVar sit_campitch( "sit_campitch", "80", FCVAR_ARCHIVE, "The camera pitch angle for the player." );

static ConVar cam_command( "cam_command", "0", FCVAR_ARCHIVE );	 // tells camera to go to thirdperson
static ConVar cam_snapto( "cam_snapto", "1", FCVAR_ARCHIVE );	 // snap to thirdperson view
...
static ConVar cam_idealdist( "cam_idealdist", "500", FCVAR_ARCHIVE );	 // thirdperson distance
static ConVar cam_collision( "cam_collision", "0", FCVAR_ARCHIVE, "When in thirdperson and cam_collision is set to 1, an attempt is made to keep the camera from passing though walls." );
static ConVar cam_showangles( "cam_showangles", "0", FCVAR_ARCHIVE, "When in thirdperson, print viewangles/idealangles/cameraoffsets to the console." );
...
static ConVar c_maxdistance( "c_maxdistance",   "1000", FCVAR_ARCHIVE );
...
void CInput::CAM_Think( void )
{
	...
	/* ChrisH */
	if ( !this->m_fCameraInThirdPerson ) 
	{
		input->CAM_ToThirdPerson();
	}
	...
	/** ChrisH we don't want cheats required for 3rd person mode
	...
	*/
	m_vecCameraOffset[ PITCH ] = sit_campitch.GetFloat();
